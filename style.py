...
_RECEIPT_COLS = [
    ...
    ("Description", "item_description"),
    ("Type", "relic_form_type"),
    # Team is here because Mike's requested report lists it; the panel is already
    # scoped to one player, but a player can hold items across several teams
    ("Team", "team"),
    ("Used status", "item_used_status"),
...
def receipt_table_html(rec, sort=None) -> str:
    ...
    for _, r in rec.iterrows():
        ...
        rows.append(
            ...
            f'<td style="text-align:left">'
            f'{escape(title_case(r["team"]) if str(r["team"]).strip() else "—")}</td>'
            f'<td style="text-align:left">'
    ...
    # QTY RECEIVED and AMOUNT are both additive -- they describe what arrived --
    # so they total straight down the column.
    #
    # QTY ON HAND is not: it is the item's CURRENT total repeated on every
    # receipt-month row, so a plain column sum would multiply-count the 34% of
    # items received in more than one month. It is totalled over DISTINCT on-hand
    # items instead.
    if rec.empty:
        tq = tqo = ta = 0
    else:
        tq = rec["qty_received"].sum()
        ta = rec["amt_received"].sum()
